//show/hide upon change of select element//
var divsO;
const showHide = (elem,num) => {
    for(var i=0; i < num; i++) {
...
//Uncheck los checkboxes cuando cambiamos de opcion//
const unchek = () =>{
...
const cambiotipo = document.getElementById("tipo");
cambiotipo.addEventListener("change",unchek);
//----------------------------------------------------------//
...
const addPedido=()=>{
    ...
    if(!cantidadchecked()){
        alert("Porfavor seleccione entre 1 y 5 "+selectTipo.value+"s");
        return;
    }
...
window.onload=function() {
    ...
    const mostrar=["descripción","nombre","numero","email","divcomuna1"];
    for(var i=0;i<5;i++){
